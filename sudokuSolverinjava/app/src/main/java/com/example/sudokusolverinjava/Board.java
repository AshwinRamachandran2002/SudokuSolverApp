...
public class Board {
    ...
    public Board(int size,String initial_data) {
        this(UUID.randomUUID(), size,initial_data);
    }

    public Board(UUID id, int size,String initial_data) {
        mId = id;
        this.size = size;
        ...
        for(int i=0;i<size;i++){
            for(int j=0;j<size;j++){
                setData(i*size+j,Integer.valueOf(initial_data.charAt(i*size+j)));
            }
        }
